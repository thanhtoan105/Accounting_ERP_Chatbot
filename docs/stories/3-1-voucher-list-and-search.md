...
- [x] Maintenance: Voucher Type activation support (follow-up fix)
  - [x] Backend: Add `PATCH /api/v1/voucher-types/{id}/activate` endpoint and `VoucherTypeService.activate(id)` implementation
  - [x] Frontend: Add "Activate" action to Voucher Types list when status is INACTIVE and wire up `activateVoucherType(id)` service
  - [x] Kept existing "Deactivate" action for ACTIVE status and refreshed list after actions

—

### Voucher Type Management (Frontend + Integration)

- [x] Frontend: Voucher Types list page
  - [x] Route `/voucher-types` with `RoleGuard` (ADMIN/CHIEF_ACCOUNTANT)
  - [x] Table columns: Type Code, Type Name, Status, Actions (Edit/Deactivate/Delete)
  - [x] Search bar with debounce and Refresh button
  - [x] Empty state prompting creation when list is blank

- [x] Frontend: Create/Edit Voucher Type dialog (shadcn)
  - [x] Fields: Type Code, Type Name (required validation with Zod + React Hook Form)
  - [x] Debit Account, Credit Account select using shadcn Select with separators
  - [x] Account list shows level 3 and all sublevels (e.g., 111, 1111, 1112, ...)
  - [x] Description textarea
  - [x] Layout: Type Code + Type Name side-by-side; Debit + Credit side-by-side
  - [x] UX: Type Code small, Type Name larger; Debit/Credit optional (can be blank)
  - [x] Error banner for API/validation errors

- [x] Frontend: Services and types
  - [x] `types/voucherType.ts` DTOs and requests
  - [x] `services/voucherType.ts` with list/get/create/update/delete/deactivate/activate
  - [x] Hooked into dialog and list actions with toast feedback

- [x] Frontend: UI polish & fixes
  - [x] Fixed Radix Select empty value error by using `value="none"` and mapping to `undefined`
  - [x] Prevented long account text overlapping by adding `w-full min-w-0` and spacing
  - [x] Added "None (Optional)" item to clear selection

- [x] Backend integration (already implemented in Epic 2/this epic tasks)
  - [x] Voucher Type API endpoints with RBAC, company scoping
  - [x] Allow optional debit/credit account IDs; DB columns nullable (Flyway V19)
  - [x] Validation: accept level 3+ accounts (code length ≥ 3)
  - [x] Deactivate and Activate endpoints for status transitions
- [x] Testing
...
Voucher UI paths updated to feature-first:

- List: `@/features/accounting/pages/Vouchers/VoucherList.tsx`
- Form: `@/features/accounting/pages/Vouchers/VoucherForm.tsx`
- Voucher components: `@/components/voucher/*`
- Services: `@/services/voucher.ts`

Routes are declared in `@/routes/AppRoutes.tsx`. Layout uses shadcn `sidebar-06` (see `@/layouts/ProtectedLayout`).
...
- 2025-11-05: Maintenance – Voucher Type activation flow added
  - Added backend endpoint `PATCH /api/v1/voucher-types/{id}/activate` and service method to re-activate previously deactivated voucher types
  - Updated frontend Voucher Types page to show an "Activate" action for INACTIVE items and call the new API; list refreshes and toasts added
  - Purpose: resolve issue where deactivated voucher types could not be activated again
...
- `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx`
...
- `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx` (added AR/AP entity column, multi-column sorting UI, improved amount display)
...
1. **AC#1 Partially Incomplete - AR/AP Entity Column Missing** [file: frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:404]
...
3. **Multi-Column Sorting Not Exposed in UI** [file: frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:182-190]
...
5. **Amount Column Displays Either Debit or Credit, Not Both** [file: frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:416-418]
...
7. **Frontend Amount Formatting Shows 0 for Zero Values** [file: frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:221-228]
...
8. **LocalStorage Persistence Doesn't Include Date Range** [file: frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:111-121]
...
| --- | --------------------------------------------------------------------------------------------------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Table shows columns: Voucher #, Date, Type, Amount, Status, Entered by, Posted by, AR/AP entity, Reversal badge, Attachment count | **PARTIAL**     | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:364-407`                                                                                                     | **AR/AP entity column missing** (HIGH severity). Amount shows either debit OR credit, not both (LOW severity). All other columns present.                                                                          |
| 2   | Filters, query text, and sort order persist for each user/company session                                                         | **IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:86-121`                                                                                                     | localStorage with companyId in storage key. Persists searchTerm, statusFilter, sortField, sortDirection, size, dateFrom, dateTo.                                                                                   |
| 3   | "Live" badges for count of draft vs posted vouchers                                                                               | **IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:239-242`, `backend/src/main/java/com/accounting/controller/voucher/VoucherController.java:136-144`        | Badges display draft/posted/unposted counts. Auto-refreshes every 30 seconds (line 178). Backend endpoint `/api/v1/vouchers/counts` implemented.                                                                   |
| 4   | Fuzzy/text/Unicode search on voucher number, description, supports Vietnamese terms                                               | **PARTIAL**     | `backend/src/main/java/com/accounting/service/impl/VoucherServiceImpl.java:86-96`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:248-261`               | Search implemented with LIKE fallback (MEDIUM severity - should use native unaccent). Frontend debounced search (300ms) works correctly.                                                                           |
| 5   | Sort by multiple columns (e.g., date DESC + status)                                                                               | **PARTIAL**     | `backend/src/main/java/com/accounting/controller/voucher/VoucherController.java:66,74-89`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:182-190`       | Backend supports multi-column sorting via `sort[]` array. Frontend only exposes single-column sorting (MEDIUM severity).                                                                                           |
...
| 7   | Lazy loading/pagination with 20-50 items per page                                                                                 | **IMPLEMENTED** | `backend/src/main/java/com/accounting/controller/voucher/VoucherController.java:60-61`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:457-468`          | Pagination supports page/size params. Default size 20, max 50. Frontend offers 20/30/50 options. Table pagination controls implemented.                                                                            |
| 8   | "No vouchers" state and UI guides users to create or adjust filters                                                               | **IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:347-358`                                                                                                    | Empty state displays helpful message (line 350-356). Conditionally shows different guidance if filters are applied vs no filters.                                                                                  |
| 9   | API error recovery: retry button, error details in toast/modal, printable JSON for support                                        | **IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:316-337,484-501`                                                                                            | Error Alert with retry button (line 321). Details button opens modal (line 326-327). Modal shows full JSON (line 488) with copy button (line 494-498).                                                             |
...
| Frontend: Create VoucherList page component                              | ✅ Complete | ✅ **VERIFIED COMPLETE** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx`                                                                                                                                                                     | Component implements table (lines 362-469), pagination (lines 457-468), badges (lines 239-242), search with debounce (lines 123-130, 248-261), filters (lines 262-313), sorting (lines 182-190), localStorage persistence (lines 86-121), empty state (lines 347-358), loading/error states (lines 340-344, 316-337). **Note:** AR/AP entity column missing (AC#1 issue). |
...
| Frontend: Implement voucher deletion with confirmation                   | ✅ Complete | ✅ **VERIFIED COMPLETE** | `frontend/src/components/voucher/DeleteVoucherDialog.tsx`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:192-213,441-451`                                                                   | Delete dialog component implements reason input (lines 93-108), validation (lines 43-47), error handling (lines 58-62). Delete button in table (lines 441-451) only enabled for draft vouchers (line 193). List refreshes on success (line 207). |
...
| AC# | Description                                                                                                                       | Status             | Evidence (file:line)                                                                                                                                                         | Notes                                                                                                                                                                                |
| --- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Table shows columns: Voucher #, Date, Type, Amount, Status, Entered by, Posted by, AR/AP entity, Reversal badge, Attachment count | **✅ IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:423-465`                                                                                                     | **All columns present including AR/AP entity placeholder (line 462, 494).** Amount column displays both debit and credit (line 475-477).                                             |
| 2   | Filters, query text, and sort order persist for each user/company session                                                         | **✅ IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:86-125`                                                                                                     | localStorage with companyId scoping. Persists searchTerm, statusFilter, sorts array, size, dateFrom, dateTo.                                                                         |
| 3   | "Live" badges for count of draft vs posted vouchers                                                                               | **✅ IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:281-284`, `backend/src/main/java/com/accounting/controller/voucher/VoucherController.java:136-144`        | Badges display draft/posted/unposted counts. Auto-refreshes every 30 seconds (line 182). Backend endpoint implemented.                                                               |
| 4   | Fuzzy/text/Unicode search on voucher number, description, supports Vietnamese terms                                               | **✅ IMPLEMENTED** | `backend/src/main/java/com/accounting/repository/VoucherRepository.java:94-101`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:305-317`                 | **Native PostgreSQL `unaccent_search()` function used** (line 96-97). Frontend debounced search (300ms, line 128-134).                                                               |
...
| 7   | Lazy loading/pagination with 20-50 items per page                                                                                 | **✅ IMPLEMENTED** | `backend/src/main/java/com/accounting/controller/voucher/VoucherController.java:60-61,69-71`, `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:518-529`    | Pagination supports page/size params. Default 20, max 50 (controller line 69-71). Frontend offers 20/30/50 options.                                                                  |
| 8   | "No vouchers" state and UI guides users to create or adjust filters                                                               | **✅ IMPLEMENTED** | `frontend/src/features/accounting/pages/Vouchers/VoucherList.tsx:404-414`                                                                                                     | Empty state displays context-aware guidance (line 410-412): different message if filters applied vs no filters.                                                                      |
