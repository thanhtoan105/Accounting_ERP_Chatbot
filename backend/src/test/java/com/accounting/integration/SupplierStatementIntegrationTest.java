...
class SupplierStatementIntegrationTest extends IntegrationTest {
  ...
  @Autowired private SupplierStatementHistoryRepository historyRepository;

  @Autowired private SupplierStatementDisputeRepository disputeRepository;
  ...
  void generateStatement_withValidRequest_returnsStatement() throws Exception {
    ...
    List<SupplierStatementHistory> history =
        historyRepository.findByCompanyId(testCompanyId, org.springframework.data.domain.PageRequest.of(0, 10))
            .getContent();
  ...
  void exportStatement_withValidId_returnsExcelFile() throws Exception {
    ...
    history.setGeneratedBy(testUserId);
    ...
    SupplierStatementHistory updatedHistory =
    ...
    verify(auditService, times(1))
        .logStatementExported(
            eq(testCompanyId), eq(testUserId), eq(history.getId()), eq("EXCEL"), any());
  }

  @Test
  void listStatements_withFilters_returnsFilteredResults() throws Exception {
    // Setup - Create multiple statement histories
    SupplierStatementHistory history1 = createStatementHistory(SupplierStatementHistory.StatementType.SUMMARY);
    SupplierStatementHistory history2 = createStatementHistory(SupplierStatementHistory.StatementType.DETAILED);
  ...
  void sendStatementToSupplier_updatesHistory() throws Exception {
    // Setup
    SupplierStatementHistory history = createStatementHistory(SupplierStatementHistory.StatementType.SUMMARY);
    history = historyRepository.save(history);
    ...
    mockMvc
        .perform(
            post("/api/v1/supplier-statements/{id}/send", history.getId())
                ...
                .header("X-Company-Id", testCompanyId.toString())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(recipientEmails)))
        .andExpect(status().isOk())
  ...
  void updateDispute_updatesStatusAndLogs() throws Exception {
    ...
    SupplierStatementDispute updatedDispute =
        disputeRepository.findById(dispute.getId()).orElseThrow();
    assertThat(updatedDispute.getStatus()).isEqualTo(SupplierStatementDispute.DisputeStatus.RESOLVED);
  ...
  void listDisputes_withFilters_returnsFilteredResults() throws Exception {
    ...
    dispute1.setStatus(SupplierStatementDispute.DisputeStatus.OPEN);
    ...
    dispute2.setStatus(SupplierStatementDispute.DisputeStatus.RESOLVED);
