...
class SalesInvoiceApprovalServiceImplTest {
    ...
    void submitForApproval_belowThreshold_doesNotRequireApproval() {
        ...
        when(vatService.validateVATSum(invoice)).thenReturn(new com.accounting.dto.VATValidationResultDTO(true));
    ...
    void approve_differentUser_succeeds() {
        ...
        verify(auditService)
                .logSalesInvoiceApproved(eq(COMPANY_ID), eq(APPROVER_ID), eq(INVOICE_ID), eq("Approved"));
    }
    ...
    void reject_withoutReason_throwsBadRequest() {
        // Arrange
        ApprovalWorkflow workflow = createWorkflow(createInvoice(ABOVE_THRESHOLD, SalesInvoiceStatus.PENDING_APPROVAL),
                CREATOR_ID);
        when(approvalWorkflowRepository.findById(WORKFLOW_ID)).thenReturn(Optional.of(workflow));
    ...
    void getPendingApprovals_returnsOnlyPendingWorkflows() {
        // Arrange
        ApprovalWorkflow pending1 = createWorkflow(createInvoice(ABOVE_THRESHOLD, SalesInvoiceStatus.PENDING_APPROVAL),
                CREATOR_ID);
        ApprovalWorkflow pending2 = createWorkflow(createInvoice(ABOVE_THRESHOLD, SalesInvoiceStatus.PENDING_APPROVAL),
                CREATOR_ID);
