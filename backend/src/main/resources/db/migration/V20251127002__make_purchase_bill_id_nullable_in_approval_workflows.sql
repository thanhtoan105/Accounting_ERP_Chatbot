...
ALTER TABLE accounting.approval_workflows
...
FOREIGN KEY (purchase_bill_id) REFERENCES accounting.purchase_bills(id) ON DELETE CASCADE;
...
COMMENT ON TABLE accounting.approval_workflows IS 'Tracks approval workflows for purchase bills (AP) and sales invoices (AR) implementing maker-checker pattern';
COMMENT ON COLUMN accounting.approval_workflows.purchase_bill_id IS 'Reference to purchase bill for AP workflows (nullable for AR workflows)';
COMMENT ON COLUMN accounting.approval_workflows.sales_invoice_id IS 'Reference to sales invoice for AR workflows (nullable for AP workflows)';
COMMENT ON CONSTRAINT chk_approval_workflows_entity_reference ON accounting.approval_workflows IS 'Ensures exactly one of purchase_bill_id or sales_invoice_id is set';
