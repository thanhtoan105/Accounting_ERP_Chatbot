...
ON accounting.voucher_lines(bank_account_id) WHERE bank_account_id IS NOT NULL;

-- Composite index for cash book queries
CREATE INDEX IF NOT EXISTS idx_voucher_lines_company_bank_account 
ON accounting.voucher_lines(company_id, bank_account_id) WHERE bank_account_id IS NOT NULL;

COMMENT ON COLUMN accounting.voucher_lines.bank_account_id IS 
