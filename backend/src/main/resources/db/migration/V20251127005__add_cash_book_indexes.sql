...
ON voucher_lines (company_id, account_id);
...
ON vouchers (company_id, status, voucher_date);
...
ON bank_accounts (company_id, active) WHERE active = true;

-- For finding bank account by GL code (cash book summary view)
CREATE INDEX IF NOT EXISTS idx_bank_accounts_company_gl_code 
ON bank_accounts (company_id, gl_account_code) WHERE gl_account_code IS NOT NULL;
