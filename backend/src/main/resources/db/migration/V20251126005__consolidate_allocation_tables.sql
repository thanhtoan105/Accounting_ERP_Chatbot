...
CREATE TABLE IF NOT EXISTS transaction_allocations (
    ...
    CONSTRAINT fk_transaction_allocations_company FOREIGN KEY (company_id) REFERENCES companies(id),
...
CREATE INDEX IF NOT EXISTS idx_transaction_allocations_company_transaction 
    ON transaction_allocations (company_id, transaction_type, transaction_id);
CREATE INDEX IF NOT EXISTS idx_transaction_allocations_document 
    ON transaction_allocations (document_type, document_id);
CREATE UNIQUE INDEX IF NOT EXISTS ux_transaction_allocations_transaction_document 
    ON transaction_allocations (transaction_id, document_id);
...
-- Step 6: Add comments
COMMENT ON TABLE transaction_allocations IS 'Unified allocation table for both AP payments and AR receipts. Links transactions to documents with allocated amounts.';
COMMENT ON COLUMN transaction_allocations.transaction_type IS 'Type of transaction: PAYMENT (AP) or RECEIPT (AR)';
COMMENT ON COLUMN transaction_allocations.transaction_id IS 'UUID of the payment or receipt';
COMMENT ON COLUMN transaction_allocations.document_type IS 'Type of document: PURCHASE_BILL or SALES_INVOICE';
COMMENT ON COLUMN transaction_allocations.document_id IS 'UUID of the purchase bill or sales invoice';
