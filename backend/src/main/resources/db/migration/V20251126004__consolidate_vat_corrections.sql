...
CREATE INDEX IF NOT EXISTS idx_vat_corrections_document 
    ON vat_corrections (document_type, document_id);
CREATE INDEX IF NOT EXISTS idx_vat_corrections_company_document 
    ON vat_corrections (company_id, document_type, document_id);

-- Step 8: Drop old ar_vat_corrections table
DROP TABLE IF EXISTS ar_vat_corrections CASCADE;

-- Step 9: Add comments for documentation
COMMENT ON TABLE vat_corrections IS 'Unified VAT corrections for both purchase bills (AP) and sales invoices (AR). Consolidated from former vat_corrections and ar_vat_corrections tables.';
COMMENT ON COLUMN vat_corrections.document_type IS 'Type of document: PURCHASE_BILL or SALES_INVOICE';
COMMENT ON COLUMN vat_corrections.document_id IS 'UUID of the parent document (purchase_bill_id or sales_invoice_id)';
