...
CREATE TABLE IF NOT EXISTS accounting.customer_code_sequences (
  company_id BIGINT NOT NULL,
  year INTEGER NOT NULL,
  last_sequence INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (company_id, year),
  FOREIGN KEY (company_id) REFERENCES accounting.companies(id)
);

-- Create index for efficient lookups
CREATE INDEX IF NOT EXISTS idx_customer_code_sequences_company_year 
  ON accounting.customer_code_sequences(company_id, year);
...
DROP FUNCTION IF EXISTS accounting.get_next_customer_code(BIGINT, INTEGER);

-- Create function in accounting schema
CREATE OR REPLACE FUNCTION accounting.get_next_customer_code(company_id_value BIGINT, year_value INTEGER)
RETURNS TEXT AS $$
DECLARE
    next_seq INTEGER;
    formatted_code TEXT;
BEGIN
    ...
    INSERT INTO accounting.customer_code_sequences (company_id, year, last_sequence)
    VALUES (company_id_value, year_value, 0)
    ON CONFLICT (company_id, year) DO NOTHING;
    
    -- Update with locked row to ensure thread-safety
    UPDATE accounting.customer_code_sequences
...
GRANT EXECUTE ON FUNCTION accounting.get_next_customer_code(BIGINT, INTEGER) TO PUBLIC;
