...
@Component("suppliers")
public class SupplierImportHandler implements ImportHandler {

    private static final List<String> EXPECTED_HEADERS = List.of("supplier_code", "name", "tax_code", "email", "phone",
            "address", "active");
    private static final Map<String, List<String>> HEADER_ALIASES = Map.of(
            "supplier_code", List.of("supplier_code", "code"),
            "name", List.of("name"),
    ...
    private static final java.util.Set<String> SUPPORTED_FORMATS = java.util.Set.of("csv", "xlsx", "xls");
    ...
    public ImportSummary handle(MultipartFile file, ImportContext context) {
        ...
        for (int i = 0; i < parsed.requests.size(); i++) {
            SupplierCreateRequest request = parsed.requests.get(i);
            int rowNumber = parsed.rowNumbers.get(i);
            try {
                var createdSupplier = supplierService.create(request);
                auditBuilders.add(new RowAuditBuilder(rowNumber, request, createdSupplier, STATUS_SUCCESS, null));
                successCount++;
    ...
    private ParsedImport parseCsv(MultipartFile file) throws IOException {
        List<SupplierCreateRequest> requests = new ArrayList<>();
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                ...
                SupplierCreateRequest request = mapRow(tokens, rowNumber, errors);
                if (request != null) {
    ...
    private SupplierCreateRequest mapRow(String[] tokens, int rowNumber, List<ImportRowError> errors) {
        ...
        Set<ConstraintViolation<SupplierCreateRequest>> violations = validator.validate(request);
        if (!violations.isEmpty()) {
            for (ConstraintViolation<SupplierCreateRequest> violation : violations) {
                errors.add(
                        new ImportRowError(
    ...
    private record ParsedImport(
            List<SupplierCreateRequest> requests, List<Integer> rowNumbers, List<ImportRowError> errors) {
    }
}
