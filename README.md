...
2. Backend: `cd backend && mvnd spring-boot:run`
...
- Seed demo data (company + users + minimal COA):
  ...
  - Via CLI (recommended):
    - `cd backend && mvnd spring-boot:run -Dspring-boot.run.arguments=--seedDemo=true`
    - Logs will include a line like: `[DEMO] Bootstrap executed. created=true, companyId=..., code=DEMO`
  - Rollback demo data:
    - `cd backend && mvnd flyway:migrate -Dflyway.target=R__remove_demo_data.sql`
