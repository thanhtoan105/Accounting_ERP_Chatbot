...
export function AgingInvoiceDetailsDialog({
    ...
    const loadInvoiceDetails = async () => {
        ...
        try {
            ...
            const message = err?.response?.data?.message || err?.message || 'Unable to load invoice details'
    ...
    const columns = useMemo<ColumnDef<AgingInvoiceDetail>[]>(
        () => [
            ...
            },
            {
                id: 'paymentCount',
                header: 'Receipts',
                cell: ({ row }) => <div className="text-center">{row.original.paymentHistory?.length || 0}</div>,
            },
        ],
        [],
    )
