...
import { Upload, X, FileText, Image as ImageIcon, AlertCircle, Loader2, Download } from 'lucide-react'
...
export function ReceiptAttachmentDropzone({
    ...
    const totalExistingSize = (existingAttachments || []).reduce((sum, att) => sum + (att.fileSize || 0), 0)
    ...
    const uploadFile = useCallback(
        async (attachmentFile: AttachmentFile) => {
            ...
            try {
                const uploaded = await uploadReceiptAttachment(receiptId, attachmentFile.file, (progress) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
                <span>
                    {(existingAttachments || []).length} of {maxFiles} files ({formatFileSize(totalExistingSize)} of{' '}
            ...
            >
                <CardContent className="p-6">
                    <div className="flex flex-col items-center justify-center gap-4 text-center">
                        ...
                        {!receiptId && (
                            <p className="text-xs text-orange-600">Save the receipt first to enable attachments</p>
                        )}
                    </div>
                </CardContent>
            </Card>
