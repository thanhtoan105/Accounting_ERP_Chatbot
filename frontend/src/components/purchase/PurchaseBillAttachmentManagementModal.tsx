...
import {
  getPurchaseBillAttachments,
  downloadPurchaseBillAttachment,
  previewPurchaseBillAttachment,
  deletePurchaseBillAttachment,
} from '@/services/purchaseBill'
import type { PurchaseBillAttachmentDTO } from '@/types/attachment'

interface PurchaseBillAttachmentManagementModalProps {
  billId: string
  open: boolean
  onOpenChange: (open: boolean) => void
  onAttachmentDeleted?: () => void
  canDelete?: boolean // Only true for DRAFT bills by creator or admin
}

export function PurchaseBillAttachmentManagementModal({
  billId,
  open,
  onOpenChange,
  onAttachmentDeleted,
  canDelete = false,
}: PurchaseBillAttachmentManagementModalProps) {
  const [attachments, setAttachments] = useState<PurchaseBillAttachmentDTO[]>([])
  ...
  const loadAttachments = async () => {
    setLoading(true)
    try {
      const data = await getPurchaseBillAttachments(billId)
      setAttachments(data)
    } catch (error: any) {
  ...
  const handleDownload = async (attachment: PurchaseBillAttachmentDTO) => {
    try {
      await downloadPurchaseBillAttachment(billId, attachment.id)
  ...
  const handlePreview = async (attachment: PurchaseBillAttachmentDTO) => {
    try {
      // For images and PDFs, we can preview
      if (attachment.mimeType.startsWith('image/') || attachment.mimeType === 'application/pdf') {
        const url = await previewPurchaseBillAttachment(billId, attachment.id)
        setPreviewUrl(url)
        // Open in new window
  ...
  const handleDeleteClick = (attachment: PurchaseBillAttachmentDTO) => {
    if (!canDelete) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deletePurchaseBillAttachment(billId, selectedAttachment.id, deleteReason.trim())
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Purchase Bill Attachments</DialogTitle>
          </DialogHeader>
