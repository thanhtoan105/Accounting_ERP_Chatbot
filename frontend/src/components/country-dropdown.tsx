...
const CountryDropdownComponent = (
  ...
  useEffect(() => {
    // If parent doesn't provide a defaultValue, do not override user's selection
    if (!defaultValue) {
      return
    }
  ...
  const triggerClasses = cn(
    'flex h-9 items-center justify-between whitespace-nowrap rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 hover:bg-secondary/80',
    slim === true && 'gap-1 w-min',
    inline ? 'rounded-r-none border-r-0 gap-1 pr-1 w-min' : 'w-full',
    className,
  )
              ...
              <>
                ...
                {inline ? (
                  <span className="text-xs text-foreground">
                    {selectedCountries[0].countryCallingCodes?.[0] || selectedCountries[0].alpha2}
                  </span>
                ) : (
                  slim === false && (
                    <span className="overflow-hidden text-ellipsis whitespace-nowrap">
