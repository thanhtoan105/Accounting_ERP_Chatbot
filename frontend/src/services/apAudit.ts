...
export const apAuditService = {
  getTimeline: (
    ...
    page: number,
    size: number,
  ): Promise<Page<APAuditTimelineDTO>> => {
    return axios
  ...
  ): Promise<AbuseDetectionResultDTO[]> => {
    return axios
      .get(`${BASE_URL}/abuse`, {
  ...
  getUnauthorizedAttempts: (
    ...
    return axios
      .get(`${BASE_URL}/unauthorized-attempts`, {
        params: { ...filters, page, size },
      })
      .then((res) => res.data)
  },
  ...
  downloadBackup: (backupId: number): Promise<Blob> => {
    return axios
      .get(`${BASE_URL}/backups/${backupId}/download`, {
        responseType: 'blob',
      })
      .then((res) => res.data)
  },

  purgeAuditLogs: (criteria: {
    userId?: number
    beforeDate: string
  }): Promise<{ purgedCount: number; status: string }> => {
