...
export const supplierStatementService = {
  ...
  async getStatementById(
    statementId: string
  ): Promise<SupplierStatementHistory> {
  ...
  async listStatements(params: StatementFilterParams): Promise<{
    ...
    if (params.supplier !== undefined)
      queryParams.set("supplier", String(params.supplier));
    if (params.startDate) queryParams.set("startDate", params.startDate);
    if (params.endDate) queryParams.set("endDate", params.endDate);
    if (params.statementType)
      queryParams.set("statementType", params.statementType);
    if (params.sort) {
  ...
  async exportStatement(
    ...
    const response = await fetchWithAuth(
      `${API_BASE}/${statementId}/export?format=${format}`
    );
  ...
  async sendBatchStatements(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to send batch statements: ${response.statusText}`
      );
    }
  },
  ...
  async downloadBatchStatements(statementIds: string[]): Promise<Blob> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to download batch statements: ${response.statusText}`
      );
    }
  ...
  async saveReconciliation(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to save reconciliation: ${response.statusText}`
      );
    }
  },
  ...
  async getStatementHistory(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch statement history: ${response.statusText}`
      );
    }
