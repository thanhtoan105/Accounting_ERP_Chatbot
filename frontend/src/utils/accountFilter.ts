...
export function getAccountFilterForVoucherType(
  ...
  switch (voucherType) {
    case 'Cash Payment':
      ...
      return (account: ChartOfAccount) => account.active === true && account.code.startsWith('111')
    case 'Bank Payment':
      // Filter by account codes starting with 112 (bank accounts)
      return (account: ChartOfAccount) => account.active === true && account.code.startsWith('112')
    case 'Cash Receipt':
    case 'Bank Receipt':
      // Filter by account codes starting with 131 (customer receivables)
      return (account: ChartOfAccount) => account.active === true && account.code.startsWith('131')
    default:
      return (account: ChartOfAccount) => account.active === true
  }
}
