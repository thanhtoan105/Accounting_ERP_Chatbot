...
export default function EditUserDialog({ open, onClose, onSuccess, user }: EditUserDialogProps) {
                ...
                const fetchUser = async () => {
                    try {
                        ...
                        reset({
                            ...
                            status: (freshUser.status?.toUpperCase() || 'ACTIVE') as 'ACTIVE' | 'INACTIVE' | 'LOCKED',
                        })
                        setFormError(null)
                    } catch (err) {
    ...
    const onSubmit = async (values: EditUserFormValues) => {
        ...
        try {
            ...
            if (canEditStatus && canManageTargetUser && values.status !== userData.status?.toUpperCase()) {
            ...
            <DialogContent className="sm:max-w-[500px]">
                <form onSubmit={handleSubmit(onSubmit)} noValidate>
                    ...
                    <DialogFooter className="gap-2">
                        <Button type="button" variant="outline" onClick={handleClose} disabled={isSubmitting || fetching}>
                            Cancel
                        </Button>
