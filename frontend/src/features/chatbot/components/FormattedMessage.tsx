...
export function FormattedMessage({ content, className }: FormattedMessageProps) {
  ...
  return <div className={className} dangerouslySetInnerHTML={{ __html: formattedContent }} />
}
...
function formatMarkdown(text: string): string {
  ...
  html = html.replace(
    /(?:^|<br \/>)\s*[-*]\s+(.+?)(?=<br \/>|$)/g,
    '<br /><span class="flex gap-2"><span>•</span><span>$1</span></span>',
  )

  // Numbered list: 1. item, 2. item
  html = html.replace(
    /(?:^|<br \/>)\s*(\d+)\.\s+(.+?)(?=<br \/>|$)/g,
    '<br /><span class="flex gap-2"><span>$1.</span><span>$2</span></span>',
  )
