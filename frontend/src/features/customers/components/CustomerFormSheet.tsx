...
const createCustomerFormSchema = (isEditMode: boolean) =>
  z.object({
    code: isEditMode
...
export default function CustomerFormSheet({
  ...
  // Load customer data for edit mode (prefer prop to avoid extra fetch in tests)
  useEffect(() => {
  ...
  const onSubmit = async (values: CustomerFormValues) => {
    ...
    try {
      ...
      const errorMessage = err?.error?.message || err?.message || 'Failed to save customer'
      if (err?.status === 409) {
        // Duplicate error
        setDuplicateError(errorMessage)
        toast.error('Duplicate customer detected', { description: errorMessage })
      } else {
        setFormError(errorMessage)
        toast.error('Failed to save customer', { description: errorMessage })
      }
    }
  }
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-4">
              ...
              <FieldGroup className="space-y-4">
                ...
                {/* Customer Code - Editable in edit mode, optional in create */}
                {isEditMode && (
                  <Field className="gap-2">
                    ...
                    <FieldContent>
                      <Input
                        id="code"
                        placeholder="e.g., CUST-2025-0001 or any custom format"
                        {...register('code')}
                      ...
                      <p className="text-xs text-muted-foreground mt-1.5">
                        You can use any format for the customer code
                      </p>
                    </FieldContent>
                  </Field>
                )}
                  ...
                  <Field className="gap-2">
                    ...
                    <FieldContent>
                      <div className="relative">
                        <Input
                          id="code"
                          placeholder="Leave empty to auto-generate (CUST-YYYY-NNNN)"
                          {...register('code')}
                          aria-invalid={!!errors.code}
                          disabled={isSubmitting}
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">
                          Linked AR data will not be deleted when deactivating a customer
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                        ...
                        >
                          {field.value ? (
                            <span className="text-green-600 dark:text-green-400">
                              Active - Customer is enabled
                            </span>
                          ) : (
                            <span className="text-muted-foreground">
                              Inactive - Customer is disabled
                            </span>
                          )}
                        </label>
                      </div>
