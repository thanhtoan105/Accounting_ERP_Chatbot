...
import { importCustomers } from '@/features/customers/services/customer'
import type { ImportResult, ImportError } from '@/types/customer'
    ...
    try {
      ...
      if (result.errorCount === 0) {
        toast.success('Import completed successfully', {
          description: `${result.successCount} customers imported.`,
        })
      ...
      const errorMessage = err?.error?.message || err?.message || 'Failed to import customers'
      setError(errorMessage)
      toast.error('Failed to import customers', { description: errorMessage })
    } finally {
      setUploading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel (.xlsx, .xls) or CSV file to import customers. Download the template for
            the correct format.
          </DialogDescription>
        </DialogHeader>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium">
                        {importResult.successCount} customers imported successfully
                      </p>
                      {importResult.errorCount > 0 && (
