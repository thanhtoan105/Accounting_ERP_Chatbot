...
import type { Supplier, SupplierQueryParams } from '@/types/supplier'
...
import SupplierDetailsPanel from '@/features/suppliers/components/SupplierDetailsPanel'
import SupplierImportWizard from '@/features/suppliers/components/SupplierImportWizard'

// Polling interval: 5 minutes default, configurable via environment variable
const POLLING_INTERVAL = import.meta.env.VITE_SUPPLIER_POLLING_INTERVAL
  ? parseInt(import.meta.env.VITE_SUPPLIER_POLLING_INTERVAL, 10)
...
export default function Suppliers() {
  ...
  const loadSuppliers = useCallback(async () => {
    try {
      ...
      const errorMessage = err?.error?.message || err?.message || 'Failed to load suppliers'
      setError(errorMessage)
      toast.error('Failed to load suppliers', { description: errorMessage })
    } finally {
      setLoading(false)
    }
  }, [page, pageSize, sortBy, sortOrder, debouncedSearch, statusFilter])

  // Immediate fetch helper for search to satisfy tests
  const loadSuppliersImmediate = async (searchOverride: string) => {
    try {
      const params: SupplierQueryParams = {
        page: 1,
        size: pageSize,
  ...
  const handleDeactivateClick = async (supplier: Supplier) => {
    try {
  ...
  const handleActivateClick = async (supplier: Supplier) => {
    try {
      ...
      const errorMessage = err?.error?.message || err?.message || 'Failed to activate supplier'
      toast.error('Failed to activate supplier', { description: errorMessage })
    }
  }
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await exportSuppliers('excel', params)
      ...
      const errorMessage = err?.error?.message || err?.message || 'Failed to export suppliers'
      toast.error('Failed to export suppliers', { description: errorMessage })
    }
  }
  ...
  const sortedSuppliers = useMemo(() => {
    return [...activeSuppliers, ...inactiveSuppliers]
  }, [activeSuppliers, inactiveSuppliers])
            ...
            <div className="text-right">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  {supplier.active ? (
                    <DropdownMenuItem onClick={() => handleDeactivateClick(supplier)}>
                      <Ban className="mr-2 h-4 w-4" />
                      Deactivate
                    </DropdownMenuItem>
                  ) : (
                    <DropdownMenuItem onClick={() => handleActivateClick(supplier)}>
                  ...
                  table.getRowModel().rows.map((row) => {
                    ...
                    return (
                      <TableRow
                        ...
                        className={!supplier.active ? 'opacity-60' : ''}
                      >
